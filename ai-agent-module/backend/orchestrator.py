...
from .tools import TOOL_REGISTRY, search_location, find_safe_route, get_nearby_emergency_services, get_crime_data, calculate_route
...
class AIAgentOrchestrator:
    ...
    async def execute_tool(
        ...
        """Executes a single tool via its HTTP wrapper and logs the audit trail."""
        ...
        try:
            # Pass http client and optional user context
    ...
    async def run(
        ...
        """
        Processes a user query by executing native LLM tool calling or deterministic intent pipeline.
        """
        exec_logger = AgentExecutionLogger()
        user_ctx = user_location or {}

        # 1. Try Native LLM Tool Calling if API key is provided
        ...
        # 2. Deterministic Intent & Entity Engine Fallback (Zero Hallucination, 100% Reliable)
    ...
    async def _run_anthropic_tool_loop(
        ...
        """Anthropic Claude tool-calling iteration loop."""
    ...
    async def _run_openai_tool_loop(
        ...
        """OpenAI / Groq function-calling iteration loop."""
    ...
    async def _run_deterministic_intent_pipeline(
        ...
        """
        Deterministic intent resolution engine when LLM APIs are not available or for instant execution.
        Follows strict multi-step orchestration:
        1. Parse intent & entities (source, dest, preference, service_type).
        2. Geocode entities via search_location tool.
        3. Handle ambiguous/missing entities with clarifying question.
        4. Execute downstream routing or emergency query.
        5. Invoke AI Briefing to synthesize natural language response.
        """
        msg_lower = message.lower().strip()

        # Check for Emergency POI Search Intent
        ...
        if any(kw in msg_lower for kw in emergency_keywords) and not any(r in msg_lower for r in ["route", "se ", "jaana", "to "]):
            ...
            # Geocode location or use current location
        ...
        # Default / Safe Routing Intent (e.g. "Mujhe Ghaziabad se Noida jaana hai safest route se")
        ...
        # Step 3: Run Three-Route Comparison
        ...
        if "safest" not in route_compare_res:
            # Fallback to standard route calculation if comparison had an issue
        ...
        # Step 4: Synthesize safety briefing
        briefing_payload = {
    ...
    async def _call_ai_briefing_module(self, payload: Dict[str, Any]) -> str:
        """Invokes AI Briefing Module POST /api/ai/briefing or generates structured summary."""
    ...
    def _extract_route_endpoints(self, text: str) -> tuple[Optional[str], Optional[str]]:
        """Parses origin and destination from English, Hindi, and Hinglish queries."""
        # Pattern 1: "Ghaziabad se Noida" / "delhi se gurgaon"
